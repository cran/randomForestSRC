                        ...
                        ...)
{
  ...
  if (conservative == TRUE) {
    minDepthParseObj <- minDepthParse(result, max.order)
    result$threshold <- minDepthParseObj$first.moment
    result$threshold.1se <- result$threshold + minDepthParseObj$sd / sqrt(numSamp)
    result$density <- minDepthParseObj$prob
  }
  else {
    minDepthParseObj <- minDepthParse(result, 0)
    #continuity correction due to using a single tree
    threshold.con <- minDepthParse(result, 1)$first.moment
    threshold.lib <- mean(minDepthParseObj$first.moment, na.rm = TRUE)
    result$threshold <- ifelse(threshold.lib - 0.5 > threshold.con, threshold.lib - 0.5, threshold.lib)
    result$threshold.1se <- result$threshold + mean(minDepthParseObj$sd, na.rm = TRUE) / sqrt(numSamp)
    result$density <- apply(minDepthParseObj$prob, 1, mean, na.rm = TRUE)
...
## Minimal depth threshold functions
  ...
  # as well as first/second moments and robust estimate of s.d
  # treat stumps as equivalent to trees with tree depth = 1.
  maxDepthStat <- function(p, D=NULL, l=NULL) {
  ...
  minDepthParse <- function(v, max.order) {
    ...
    if (max.order > 0) {
      ...
      maxDepthStatObj <- maxDepthStat(p, l = l)
      first.moment <- maxDepthStatObj$first.moment
      second.moment <- maxDepthStatObj$second.moment
      sd.robust <- maxDepthStatObj$sd.robust
      prob <- maxDepthStatObj$prob
      ...
      prob <- sapply(1:ntree, function(tree) {
        ...
        c(maxDepthStat(p, l = l)$prob, rep(0, maxTreeHeight - treeHeight[tree] * (1 + 1 * (treeHeight[tree] == 1))))
      })

      # extract the tree threshold value
      first.moment <- unlist(sapply(1:ntree, function(tree) {
        l <- nodes.at.depth[, tree][1:sum(nodes.at.depth[, tree] > 0, na.rm = TRUE)]
        maxDepthStat(p, l = l)$first.moment
      }))

      # extract the tree second moment
      second.moment <- unlist(sapply(1:ntree, function(tree) {
        l <- nodes.at.depth[, tree][1:sum(nodes.at.depth[, tree] > 0, na.rm = TRUE)]
        maxDepthStat(p, l = l)$second.moment
      }))
      
      # extract the tree robust standard deviation
      sd.robust  <- unlist(sapply(1:ntree, function(tree) {
        l <- nodes.at.depth[, tree][1:sum(nodes.at.depth[, tree] > 0, na.rm = TRUE)]
        maxDepthStat(p, l = l)$sd.robust
      }))
      
    }
                                    
    return(list(prob = prob, first.moment = first.moment,
                sd = sqrt(second.moment - first.moment^2), sd.robust = sd.robust))
